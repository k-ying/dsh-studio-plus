...
> 本分支支持 0.1.2+，并允许你自己选择版本——Environment 面板列出所有已发布版本，
> 每个候选版本先装入隔离的 staging 运行时并验证启动，验证失败自动保留原运行时。
...
**用版本选择器替代钉死的版本号。** 上游 0.9.8 采纳了这个思路：Environment 面板
列出所有已发布的 dsh 版本，每个候选版本先装入隔离的 staging 运行时、独立完成启动
验证，通过后才转正——验证失败自动保留原运行时。本 fork 保留该引擎及上述豁免，
因此任何可选版本都能正常连接后端，不再报 `dsh web authentication required`。内置
版本为 dsh 0.1.2-rc.1（上游仍钉 0.1.1-rc.2）。
