...
- 回环 cookie 的清扫从「应用启动时一次」改为「每个 harness 进程启动前」。
  dsh 每次启动都会种一个随机命名、30 天有效的新鉴权 cookie，且从不清理旧的；
  开着的 Studio 窗口一天里会重启 harness 多次，而 harness 用的是原生
  `node:http` 服务器，默认请求头上限 16 KiB——大约七十个这种 cookie 就到顶。
- harness 公告的地址若写成主机名（`http://localhost:…`），现在会改写为
  `127.0.0.1`，也就是外壳自己所在的地址。对 cookie 来说 `localhost` 与
  `127.0.0.1` 是两个站，否则 iframe 会退回第三方上下文，最初的 401 会原样
  回来、且没有别的症状。引导令牌在改写中保留。

## [0.9.8-plus3] — 2026-09-21

### 修复

- 壳服务器的请求头改为流式排空（只保留请求行，其余丢弃，1MB 兜底）。dsh 每次
  启动都会在 127.0.0.1 上种一个随机命名、30 天有效的鉴权 cookie（cookie 不按
  端口隔离），积累一天后请求头超过原 4096 字节上限，服务器在请求发完前关闭
  连接，WebKit 报 -1005，应用无法启动。现在请求头多大都不影响。
- 启动时清扫 webview 里 127.0.0.1/localhost 的历史 cookie（在 harness 启动
  之前）。保护 harness 自身不被旧 cookie 堆撑爆其 16KB 请求头限制。
- 在 Studio 集成 patch 层为 dshmarket 设置 `allowRestart: false`：市场插件的
  一键自重启会 spawn 一个脱离 supervisor 的分离进程，每次装卸插件都留下一个
  共享同一数据目录的僵尸实例（表现为切模型/切会话卡死）。现在市场的重启横幅
  会提示手动重启，由 Studio 统一管理。
...
- 采用上游 0.9.8 的 Harness 版本选择器：Environment 面板列出全部已发布的 dsh
  版本；每个候选版本先装入隔离的 staging 运行时、独立完成启动验证，通过后才
  转正——验证失败自动保留原运行时。选择器引擎取代本 fork 0.9.22 的通道实现；
  内置版本升级为 dsh 0.1.2-rc.1（上游仍钉 0.1.1-rc.2）。
- 保留本 fork 的浏览器会话豁免：0.1.2+ 的鉴权门禁会拒绝内嵌 Studio 窗口——
  webview 的 iframe 无法保存引导 cookie（SameSite=Strict）。受管安装时对
  Studio 启动的 harness（`DSH_DESKTOP=1`）豁免该门禁；终端直接运行时 token
  鉴权保持原样。上游冒烟脚本中"可交换 SameSite=None cookie"的检查被此豁免
  取代——已发布的 dsh 版本发的都是 Strict cookie，该检查必然失败。
- Studio 集成客户端现在同时兼容两代 Harness：0.1.1 的 `ctx.workspaces` 直调
  与 0.1.2 的 Result 包装服务 + `uiWorkspace.startSession`，通过特性检测自动
  选择。
- fork CI 一并迁移：发布产物指向本 fork 的更新器端点；质量门与运行时冒烟
  脚本从运行时合约读取钉定版本，不再依赖过时常量；单元测试使用隔离数据
  根目录，开发者本机 pin 的版本不会把测试套件弄红。

## [0.9.7-plus1] — 2026-09-14

### 变更

- 合并上游 0.9.3 至 0.9.7：Tauri dialog/opener 插件刷新、兼容 Node 24 的 CI
  actions、面向任务的入门文档、支持矩阵与故障排除指南。0.9.21–0.9.22 引入的
  Harness 版本通道、预检与 0.1.2 支持不受影响——上游仍钉 dsh 0.1.1-rc.2，
  因此本 fork 内置版本保持 0.1.2-rc.1。

## [0.9.7-plus1] —— 2026-09-14

### 变更

- 合并上游 0.9.3 至 0.9.7：Tauri dialog/opener 插件更新、兼容 Node 24 的 CI
  actions、面向任务的入门文档、支持矩阵与故障排除指南。0.9.21–0.9.22 引入的
  Harness 版本通道、预检与 0.1.2 支持不受影响——上游仍钉在 dsh 0.1.1-rc.2，
  因此本分支的内置版本保持 0.1.2-rc.1。

## [0.9.22] —— 2026-09-10

### 新增

- Harness 版本不再编译期钉死。设置 → dsh 运行时列出所有已发布的 dsh 版本
  （可从 npm registry 手动刷新），当 `dist-tags/latest` 超过已安装版本时显示
  更新徽标，切换走既有的托管安装流程。
- 版本切换带预检：先从 registry 拉取 Studio 会打补丁的两个包并检查缝合点，
  然后才安装。鉴权机制已变化的版本会被拒绝；只是无法应用目录选择器增强的版本
  降级安装，跳过的补丁组记录在运行时标记中，合约检查予以容忍。
- 「已选定但未安装」状态直接显示在选择器旁，失败或中断的切换不再无声无息。

### 变更

- 内嵌的运行时清单改为模板：跟随 Harness 锁步的 `@deepseek-ai/*` 包改写为所选
  版本，独立版本号的包保持不动；非内置版本现场解析依赖图，不再使用内嵌的
  lock 文件。
- 托管安装守卫、安装日志与崩溃恢复改为跟踪通道所选版本，而非内置版本。
- 插件兼容性按实际安装的 Harness 版本评估，而非构建时钉死的版本。

### 修复

- 预检与安装共用同一套 npm 可用 Node 的选择逻辑；所选 Node 不带 npm 不再导致
  切换在开始之前就失败。
- 未 pin 任何版本时，通道选择器正确显示内置版本，而不是 registry 列表的第一项。

## [0.9.21] —— 2026-09-07

### 新增

- 支持 Harness 0.1.2。0.1.2 的 Web 界面要求用每次启动的临时令牌换 SameSite
  Cookie，外壳的跨站 iframe 永远无法完成这套流程，因此安装器改为在 qualify 阶段
  改造连接包：由 Studio 启动的 harness（`DSH_DESKTOP`）视本机回环调用为已认证
  ——与 0.1.1 的暴露面一致。从终端启动的 `dsh` 完整保留令牌门禁。

### 变更

- 运行时合约目标为 `@deepseek-ai/dsh` 0.1.2-rc.1，包含 0.1.2 中改为 peer
  依赖的十个包。
- Studio 集成插件适配 0.1.2 的工作区架构：`dsh-client-runtime` 已移除，工作区
  创建改走 `dsh-api-workspace-controller`，会话启动移到 `dsh-client-ui-workspace`。
- 存量运行时在首次启动时自动重新 qualify（运行时 schema 3）。
### 文档
